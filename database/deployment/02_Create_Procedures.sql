...
-- =====================================================================
-- sp_pm_view_obligations_typed  (Phase 2D of the obligation taxonomy
-- rollout; mirrored from database/122_view_obligations_typed_proc.sql
-- for fresh deploys).
--
-- Standalone read-path sub-proc that projects the 7-type typed detail
-- of each obligation reachable from a given organization_requirement,
-- practice, or practice_instance context.  Called by Practice Management
-- Web via a new entity type 'evidence-obligations-typed' (see the API
-- routing in PracticeRepositoryService).  Requires Control Management
-- scripts 026 (schema) and 027 (data) to have been applied so the
-- GRAC_New taxonomy tables exist.
-- =====================================================================
    ...
    BEGIN
        ...
        ;WITH context_requirement AS (
            ...
            WHERE (@p_organization_requirement_id IS NOT NULL AND req.organization_requirement_id = @p_organization_requirement_id)
               OR (@p_practice_id IS NOT NULL AND EXISTS(SELECT 1 FROM grac_practice.practice p WHERE p.practice_id = @p_practice_id AND p.organization_requirement_id = req.organization_requirement_id))
               OR (@p_practice_instance_id IS NOT NULL AND EXISTS(SELECT 1 FROM grac_practice.practice_instance pi JOIN grac_practice.practice p ON p.practice_id = pi.practice_id WHERE pi.practice_instance_id = @p_practice_instance_id AND p.organization_requirement_id = req.organization_requirement_id))
        ),
        distinct_requirements AS (
            SELECT DISTINCT repository_requirement_id, organization_id
            FROM context_requirement WHERE repository_requirement_id IS NOT NULL
        ),
        distinct_obligations AS (
            ...
            JOIN GRAC_New.obligation_requirement_release_map orm
                ON orm.requirement_id = dr.repository_requirement_id AND orm.status = N''Active''
        )
        SELECT
            rel.release_id AS FrameworkReleaseId, rel.FrameworkRelease,
            ...
            o.approval_authority AS ApprovalAuthority, o.responsibility AS Responsibility,
            COALESCE((SELECT s.state_rule_id AS Id, s.attribute, s.operator, s.[value], s.unit, s.tolerance, s.remarks FROM GRAC_New.obligation_state_rule s WHERE s.obligation_id = o.obligation_id AND s.status = N''Active'' FOR JSON PATH), N''[]'') AS StateRulesJson,
            COALESCE((SELECT es.execution_spec_id AS Id, es.[action], es.execution_frequency_id AS ExecutionFrequencyId, ef.option_label AS ExecutionFrequency, es.trigger_condition, es.responsible_party, es.due_within, es.remarks FROM GRAC_New.obligation_execution_spec es LEFT JOIN GRAC_New.reference_option ef ON ef.reference_option_id = es.execution_frequency_id WHERE es.obligation_id = o.obligation_id AND es.status = N''Active'' FOR JSON PATH), N''[]'') AS ExecutionSpecsJson,
            COALESCE((SELECT [as].assurance_spec_id AS Id, [as].verification_method, [as].scope, [as].assurance_frequency_id AS AssuranceFrequencyId, af.option_label AS AssuranceFrequency, [as].assurance_party, [as].remarks FROM GRAC_New.obligation_assurance_spec [as] LEFT JOIN GRAC_New.reference_option af ON af.reference_option_id = [as].assurance_frequency_id WHERE [as].obligation_id = o.obligation_id AND [as].status = N''Active'' FOR JSON PATH), N''[]'') AS AssuranceSpecsJson,
            COALESCE((SELECT er.event_response_id AS Id, er.trigger_event, er.response_action, er.sla_value AS SlaValue, er.sla_unit AS SlaUnit, er.escalation_path, er.remarks FROM GRAC_New.obligation_event_response er WHERE er.obligation_id = o.obligation_id AND er.status = N''Active'' FOR JSON PATH), N''[]'') AS EventResponsesJson,
            COALESCE((SELECT cr.constraint_rule_id AS Id, cr.prohibited_condition, cr.scope, cr.exception_policy, cr.remarks FROM GRAC_New.obligation_constraint_rule cr WHERE cr.obligation_id = o.obligation_id AND cr.status = N''Active'' FOR JSON PATH), N''[]'') AS ConstraintRulesJson,
            COALESCE((SELECT rs.retention_spec_id AS Id, rs.retained_object, rs.min_retention_value AS MinRetentionValue, rs.min_retention_unit AS MinRetentionUnit, rs.max_retention_value AS MaxRetentionValue, rs.max_retention_unit AS MaxRetentionUnit, rs.disposal_policy, rs.remarks FROM GRAC_New.obligation_retention_spec rs WHERE rs.obligation_id = o.obligation_id AND rs.status = N''Active'' FOR JSON PATH), N''[]'') AS RetentionSpecsJson,
            COALESCE((
                SELECT * FROM (
                    SELECT N''Direct'' AS Source, NULL AS LinkTypeCode,
                        roe.obligation_evidence_id AS ObligationEvidenceId, roe.evidence_type_id AS EvidenceTypeId,
                        et.evidence_type_name AS EvidenceType, roe.frequency_id AS FrequencyId, freq.option_label AS Frequency,
                        roe.retention_requirement AS RetentionRequirement, roe.remarks AS Remarks, et.display_order AS DisplayOrder
                    ...
                    UNION ALL
                    SELECT N''Link'', l.TypeCode, roe2.obligation_evidence_id, roe2.evidence_type_id, et2.evidence_type_name,
                           roe2.frequency_id, freq2.option_label, roe2.retention_requirement, roe2.remarks, et2.display_order
                    FROM (
                        SELECT N''State'' AS TypeCode, obligation_id, obligation_evidence_id, status FROM GRAC_New.obligation_state_evidence_link
                        UNION ALL SELECT N''Execution'', obligation_id, obligation_evidence_id, status FROM GRAC_New.obligation_execution_evidence_link
                        UNION ALL SELECT N''Assurance'', obligation_id, obligation_evidence_id, status FROM GRAC_New.obligation_assurance_evidence_link
                        UNION ALL SELECT N''EventResponse'', obligation_id, obligation_evidence_id, status FROM GRAC_New.obligation_event_response_evidence_link
                        UNION ALL SELECT N''Constraint'', obligation_id, obligation_evidence_id, status FROM GRAC_New.obligation_constraint_evidence_link
                        UNION ALL SELECT N''Retention'', obligation_id, obligation_evidence_id, status FROM GRAC_New.obligation_retention_evidence_link
                    ) l
                    ...
                    WHERE l.obligation_id = o.obligation_id AND l.status = N''Active''
                ) combined
                ORDER BY combined.Source, ISNULL(combined.DisplayOrder, 999), combined.EvidenceType
                FOR JSON PATH
            ), N''[]'') AS EvidenceJson
        FROM distinct_obligations dob
        JOIN GRAC_New.requirement_obligation o ON o.obligation_id = dob.obligation_id AND o.status = N''Active''
        ...
        OUTER APPLY (
            SELECT TOP 1 r.release_id,
                COALESCE(a.artifact_code + N'' '' + r.version_no, a.artifact_name + N'' '' + r.version_no, r.version_no) AS FrameworkRelease
            FROM distinct_requirements dr2
            JOIN GRAC_New.obligation_requirement_release_map orm2 ON orm2.requirement_id = dr2.repository_requirement_id AND orm2.obligation_id = dob.obligation_id AND orm2.status = N''Active''
        ...
        ORDER BY ISNULL(t.display_order, 999), rel.FrameworkRelease, COALESCE(o.obligation_name, o.obligation_text)
...
ELSE
BEGIN
    PRINT 'sp_pm_view_obligations_typed skipped -- GRAC_New.obligation_type_master missing (apply Control Management scripts 026 + 027 first, then rerun deployment).';
END
GO
