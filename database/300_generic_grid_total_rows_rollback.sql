-- =====================================================================
-- 300_generic_grid_total_rows_rollback.sql
--
-- Restores dbo.pm_get_practice_repository to its 002 definition --
-- identical to 300 except that the 37 paged branches lose
-- COUNT(*) OVER () AS TotalRows.
--
-- ---------------------------------------------------------------------
-- WHAT BREAKS IF YOU RUN THIS
-- ---------------------------------------------------------------------
-- Every generic-grid screen loses its row range and its Next bound.
-- pm-grid degrades rather than failing -- setTotal(undefined) leaves the
-- total null, the label falls back to "Page N", and Next reverts to the
-- row-count guess -- so the grids keep working, exactly as badly as they
-- did before 300. Nothing errors; the pager simply goes back to being
-- wrong on a full last page.
--
-- The Web and API tiers need no rollback of their own. practice.js reads
-- TotalRows defensively and the C# fallbacks are additive in the same
-- way, so both tolerate the column's absence.
--
-- NO SCHEMA CHANGE. NO DATA CHANGE, in either direction. 300 replaced
-- one procedure and wrote no row.
-- =====================================================================
