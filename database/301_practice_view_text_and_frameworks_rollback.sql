-- =====================================================================
-- 301_practice_view_text_and_frameworks_rollback.sql
--
-- Restores both procedures to their pre-301 definitions:
--   dbo.sp_pm_view_obligations_typed        -> 224 body (no ObligationText)
--   grac_practice.sp_practice_detail_get    -> 218 body (no MappedFrameworksJson)
--
-- ---------------------------------------------------------------------
-- WHAT BREAKS IF YOU RUN THIS
-- ---------------------------------------------------------------------
-- Nothing errors. Both additions were read defensively by the Practice
-- View page:
--   * an obligation card without ObligationText renders the name alone,
--     which is what it did before 301;
--   * Practice Details with no MappedFrameworksJson omits the
--     Frameworks row entirely, the same way it omits any other detail
--     row whose value is empty.
-- The page simply loses the two pieces of information again.
--
-- No Web or API rollback is needed. PracticeDetail.MappedFrameworksJson
-- is read through a null-tolerant reader and the renderer treats a
-- missing value as "nothing mapped".
--
-- NO SCHEMA CHANGE. NO DATA CHANGE, in either direction. 301 replaced
-- two procedures and wrote no row.
-- =====================================================================
SET NOCOUNT ON;
GO
