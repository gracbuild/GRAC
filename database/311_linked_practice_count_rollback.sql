-- =====================================================================
-- 311 LinkedPracticeCount -- ROLLBACK
--
-- Restores sp_risk_register_get to 310's body: RiskVersion stays,
-- LinkedPracticeCount goes.
--
-- Nothing else to undo. 311 added no table, no column and no data, and
-- MappedPracticeCount was never altered -- so this is one CREATE OR
-- ALTER and no data is at risk either way.
--
-- AFTER RUNNING THIS, Risk Context reverts to reporting 0 practices for
-- any risk whose Primary map row has not been derived yet. That is the
-- behaviour 311 exists to fix; roll back only if the extra column is
-- itself causing a problem.
--
-- Re-runnable: yes.
...
BEGIN
    PRINT 'ABORT (311 rollback): sp_risk_register_get missing. Nothing to undo.';
    SET NOEXEC ON;
END
GO
...
BEGIN
    PRINT 'ABORT (311 rollback): risk_version missing -- 310 has been rolled back.';
    PRINT '             Restoring 310s body here would select a column that';
    PRINT '             does not exist. Run 310 first, or 292 to go further back.';
    SET NOEXEC ON;
END
GO
...
SELECT '311r-a LinkedPracticeCount removed' AS Check_,
       CASE WHEN OBJECT_DEFINITION(OBJECT_ID('grac_practice.sp_risk_register_get'))
                 NOT LIKE '%AS LinkedPracticeCount%'
            THEN 'PASS' ELSE 'FAIL' END AS Result
UNION ALL
SELECT '311r-b 310 RiskVersion survived',
       CASE WHEN OBJECT_DEFINITION(OBJECT_ID('grac_practice.sp_risk_register_get'))
                 LIKE '%AS RiskVersion%'
            THEN 'PASS' ELSE 'FAIL' END
UNION ALL
SELECT '311r-c 292 AcceptedByRoleNames survived',
       CASE WHEN OBJECT_DEFINITION(OBJECT_ID('grac_practice.sp_risk_register_get'))
                 LIKE '%AS AcceptedByRoleNames%'
            THEN 'PASS' ELSE 'FAIL' END;

PRINT '';
PRINT '311 rollback complete. sp_risk_register_get is back to 310s body.';
GO
