-- =====================================================================
-- 293_risk_review_frequency.sql
--
-- PURPOSE
--   A risk's acceptance records HOW OFTEN it comes back, not just when.
--   Adds risk_register.review_frequency_id, carries it through the
--   acceptance save and read, and exposes the frequency list the
--   Acceptance screen's new "Review Frequency" select is built from.
--
-- ---------------------------------------------------------------------
-- WHY frequency_master AND NOT A NEW LOOKUP
-- ---------------------------------------------------------------------
-- grac_practice.frequency_master already exists (001) and already
-- carries exactly what this feature needs:
--
--     frequency_value INT,  frequency_unit NVARCHAR(40),  is_custom BIT
--
-- so "Monthly = today + 1 month" is DATA, not a rule to hard-code in
-- JavaScript: the UI reads value+unit and adds them. is_custom marks
-- the Custom / Event-driven rows, where no date can be derived and the
-- user types one. A second frequency list would have been a second
-- place for Quarterly to mean something different.
--
-- organization_committee.review_frequency_id (002) is the same column
-- against the same master, so this follows a pattern already in the
-- schema rather than inventing one.
--
-- ---------------------------------------------------------------------
-- THE DATE IS STILL THE AUTHORITY
-- ---------------------------------------------------------------------
-- next_review_date remains NOT NULL-checked and remains what brings a
-- risk back (56601 / 56602 are untouched). The frequency is what the
-- date was DERIVED from, recorded so the next acceptance can default
-- the same way and so a report can say "annually" without inferring it
-- from two dates. A caller that sends only a date still works exactly
-- as before -- the parameter defaults to NULL.
--
-- Re-runnable: yes (COL_LENGTH guard + CREATE OR ALTER).
-- Rollback: database/293_risk_review_frequency_rollback.sql
-- DEPENDS ON: 001 (frequency_master), 264 (acceptance procs),
--             291 (fn_employee_role_names, whose column this preserves).
...
BEGIN
    PRINT 'ABORT (293): frequency_master missing. Run 001 first.';
    RAISERROR('293_risk_review_frequency: prerequisites missing.', 16, 1);
    SET NOEXEC ON;
END
GO

IF OBJECT_ID('grac_practice.sp_risk_acceptance_save','P') IS NULL
   OR OBJECT_ID('grac_practice.fn_employee_role_names','FN') IS NULL
BEGIN
    PRINT 'ABORT (293): run 264 and 291 first.';
    RAISERROR('293_risk_review_frequency: acceptance procs or 291 missing.', 16, 1);
    SET NOEXEC ON;
END
GO

-- ---------------------------------------------------------------------
-- 1. The column. Nullable: every risk accepted before this migration
--    has a review date and no frequency, and that is a legitimate
--    state, not a backfill candidate -- guessing a cadence from one
--    date would be inventing history.
-- ---------------------------------------------------------------------
IF COL_LENGTH('grac_practice.risk_register','review_frequency_id') IS NULL
BEGIN
    ALTER TABLE grac_practice.risk_register ADD review_frequency_id INT NULL;
    PRINT '293: risk_register.review_frequency_id added.';
END
GO

IF COL_LENGTH('grac_practice.risk_register','review_frequency_id') IS NOT NULL
   AND NOT EXISTS (SELECT 1 FROM sys.foreign_keys
                    WHERE name = 'fk_pm_risk_register_review_frequency')
BEGIN
    ALTER TABLE grac_practice.risk_register
        ADD CONSTRAINT fk_pm_risk_register_review_frequency
        FOREIGN KEY (review_frequency_id)
        REFERENCES grac_practice.frequency_master(frequency_id);
    PRINT '293: FK to frequency_master added.';
END
GO

-- ---------------------------------------------------------------------
-- 2. The list the Acceptance select is built from.
--    value/unit/is_custom travel with each row precisely so the client
--    derives the date from data rather than from a hard-coded switch.
-- ---------------------------------------------------------------------
CREATE OR ALTER PROCEDURE grac_practice.sp_risk_review_frequency_list
AS
BEGIN
    SET NOCOUNT ON;

    SELECT f.frequency_id    AS FrequencyId,
           f.frequency_code  AS FrequencyCode,
           f.frequency_name  AS FrequencyName,
           f.frequency_value AS FrequencyValue,
           f.frequency_unit  AS FrequencyUnit,
           f.is_custom       AS IsCustom
      FROM grac_practice.frequency_master f
     WHERE f.is_active = 1
     ORDER BY f.display_order, f.frequency_name;
END;
GO

-- =====================================================================
-- 3. sp_risk_acceptance_save -- re-issued from 264
--
-- Extracted from 264 and changed in exactly two places, both marked:
-- the new @review_frequency_id parameter, and the one column added to
-- the UPDATE. Every guard, THROW, history insert and result column is
...
-- =====================================================================
-- 4. sp_risk_acceptance_get -- re-issued from 291 (NOT from 264)
--
-- 291 added AcceptedByRoleNames to this procedure. Re-issuing 264's
-- version here would silently drop it, so the extract below is taken
-- from 291. Changed in one place: the two frequency columns, so the
-- Acceptance screen can preselect the frequency a risk already has.
-- =====================================================================
...
BEGIN
    ...
    SELECT r.risk_register_id        AS RiskRegisterId,
           ...
           -- NEW in 293. The cadence the date was derived
           -- from, plus its display name, so the select can
           -- preselect without a second lookup round trip.
           r.review_frequency_id     AS ReviewFrequencyId,
           fr.frequency_name         AS ReviewFrequencyName,
      ...
      LEFT JOIN grac_practice.frequency_master fr ON fr.frequency_id = r.review_frequency_id
...
PRINT '293: review_frequency_id added; acceptance save/get carry it; sp_risk_review_frequency_list created.';
GO
