-- =====================================================================
-- 091 Organization Assurance Plans menu seed rollback
-- =====================================================================
SET NOCOUNT ON;
GO
...
BEGIN
    UPDATE ff
    SET is_enabled = 0, updated_by = 'rollback-091', updated_dt = SYSUTCDATETIME()
    FROM grac_practice.feature_flag ff
    JOIN grac_practice.feature_flag_master fm ON fm.feature_flag_id = ff.feature_flag_id
    WHERE fm.feature_code IN (N'screen.org-assurance-plans');
END
GO
...
BEGIN
    UPDATE grac_practice.feature_flag_master
       SET is_active = 0, updated_by = 'rollback-091', updated_dt = SYSUTCDATETIME()
     WHERE feature_code IN (N'screen.org-assurance-plans');
END
GO

PRINT '091 rollback complete.';
GO
