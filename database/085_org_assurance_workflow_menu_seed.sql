-- =====================================================================
-- 085 Organization Assurance Workflow Config -- menu + permissions
--
-- Adds the fifth Phase 2 screen (BRD Part 2 Sec 6) under nav-assurance.
--
-- Rollback: 085_org_assurance_workflow_menu_seed_rollback.sql.
-- =====================================================================
SET NOCOUNT ON;
SET NOEXEC OFF;
GO
...
BEGIN PRINT 'ABORT (085): schema grac_practice missing.'; SET @prereqs_ok = 0; END
IF OBJECT_ID('grac_practice.menu_master','U') IS NULL
BEGIN PRINT 'ABORT (085): menu_master missing.'; SET @prereqs_ok = 0; END
IF OBJECT_ID('grac_practice.feature_flag_master','U') IS NULL
   OR OBJECT_ID('grac_practice.feature_flag','U') IS NULL
BEGIN PRINT 'ABORT (085): feature_flag* missing. Run 041.'; SET @prereqs_ok = 0; END
IF NOT EXISTS (SELECT 1 FROM grac_practice.menu_master WHERE menu_key = N'nav-assurance')
BEGIN PRINT 'ABORT (085): nav-assurance parent missing. Run 063.'; SET @prereqs_ok = 0; END
IF @prereqs_ok = 0
BEGIN
    RAISERROR('085: prerequisites missing.', 16, 1);
    SET NOEXEC ON;
END
GO
...
USING (VALUES
    (N'screen.org-assurance-workflow-config',
     N'Assurance Workflow Config',
     N'Phase 2 Assurance Management -- workflow config per definition version (BRD Part 2 Sec 6).')
...
WHEN MATCHED THEN UPDATE SET
    ...
    updated_by = 'seed-085', updated_dt = SYSUTCDATETIME()
WHEN NOT MATCHED BY TARGET THEN INSERT
    (feature_code, feature_name, description, category, default_enabled, entered_by)
VALUES
    (src.feature_code, src.feature_name, src.description, N'Screen', 0, 'seed-085');
GO

MERGE grac_practice.menu_master AS target
USING (VALUES
    (N'org-assurance-workflow-config',
     N'Workflow Config',
     N'Practice/org-assurance-workflow-config',
     464, N'sitemap', N'Assurance')
...
VALUES
    (source.menu_key, source.menu_name, source.menu_url, source.display_order,
     source.icon_class, source.module_type, N'Active', 'seed-085');
GO

DECLARE @nav_id BIGINT = (SELECT menu_id FROM grac_practice.menu_master WHERE menu_key = N'nav-assurance');
IF @nav_id IS NULL THROW 85001, 'nav-assurance parent menu missing. Run migration 063 first.', 1;

UPDATE grac_practice.menu_master
   SET parent_menu_id = @nav_id,
       updated_by     = 'seed-085',
       updated_dt     = SYSUTCDATETIME()
 WHERE menu_key IN (N'org-assurance-workflow-config');
GO
...
USING (
    ...
    WHERE r.role_name = N'Admin'
      ...
      AND m.menu_key IN (N'org-assurance-workflow-config')
) AS source
...
VALUES
    (source.role_id, source.menu_id, source.can_view, source.can_add,
     ...
     N'Active', source.record_status_id, 'seed-085');
GO
...
USING (
    ...
    WHERE o.status = N'Active'
      AND fm.feature_code IN (N'screen.org-assurance-workflow-config')
) AS source
ON target.organization_id = source.organization_id
   AND target.feature_flag_id = source.feature_flag_id
WHEN MATCHED THEN UPDATE SET
    is_enabled = 1,
    notes      = COALESCE(target.notes, N'Enabled by seed-085'),
    updated_by = 'seed-085', updated_dt = SYSUTCDATETIME()
WHEN NOT MATCHED THEN INSERT
    (organization_id, feature_flag_id, is_enabled, notes, entered_by)
VALUES
    (source.organization_id, source.feature_flag_id, 1,
     N'Enabled by seed-085', 'seed-085');
GO

COMMIT TRAN;
GO

SELECT 'org-assurance-workflow-config has parent_menu_id = nav-assurance' AS Check_,
       CASE WHEN EXISTS (
           SELECT 1 FROM grac_practice.menu_master c
           JOIN grac_practice.menu_master p ON p.menu_id = c.parent_menu_id
           WHERE c.menu_key = N'org-assurance-workflow-config'
             AND p.menu_key = N'nav-assurance')
            THEN 'PASS' ELSE 'FAIL' END AS Result;

PRINT '085 org-assurance-workflow-config menu seed complete.';
GO
