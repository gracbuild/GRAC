...
--
-- PURPOSE
--   Evidence rows stop depending on somebody pressing Save.
--
-- ---------------------------------------------------------------------
-- WHAT WAS WRONG
-- ---------------------------------------------------------------------
-- practice_instance_evidence rows were created in exactly one place: the
-- evidence block inside sp_resolve_obligation_adopt, which runs only when
-- the workspace posts a save. So an obligation could be adopted and show
-- nothing under Evidence, and the panel had to explain itself:
--
--     "1 evidence type is published, but no row exists here ...
--      press Save obligations to try again"
--
-- The diagnosis on the reported instance was not a catalogue problem --
-- both joins resolve, PublishedCount = 1 and CreatableTypes = 1 for all
-- three obligations. The rows were simply never inserted, because the
-- evidence was published to the repository AFTER those obligations were
-- adopted, and nothing re-runs that insert on its own.
--
-- Asking the operator to press Save to repair state they did not break is
-- not a fix. Evidence belongs under its obligation whenever the
-- obligation is adopted, in whatever state it is in.
--
-- ---------------------------------------------------------------------
-- WHAT THIS DOES
-- ---------------------------------------------------------------------
-- 1. sp_resolve_evidence_reconcile_for_instance -- NEW. Carries the
--    adopt-unattached UPDATE and the insert-missing INSERT, byte for byte
--    from 244, with ONE substitution: the driving set is the adoption
--    table rather than the call's @req payload.
--
--       FROM @req r  WHERE r.IsAdopted = 1     ->     FROM @adopted r
--
--    Everything else -- every join, the name match, the NOT EXISTS guard,
--    the column list, the literals -- is the 244 text unchanged.
--
-- 2. sp_resolve_obligation_adopt -- re-emitted from 244 with that block
--    REMOVED and an EXEC of the new procedure in its place. There is now
--    one copy of the rule, not two, and adopt keeps behaving as it does
--    today because it writes its adoption rows BEFORE this point.
--
-- The API then calls the same procedure when the workspace loads its
-- obligations, which is what makes the rows appear without a save. That
-- is a read that writes, and it is the pattern this codebase already
-- uses: QuerySubscribedFrameworksAsync and QueryReleaseStatementsAsync
-- both call SyncOrganizationFrameworkStatementsAsync before querying.
--
-- ---------------------------------------------------------------------
-- WHY IT IS SAFE TO RUN ON EVERY LOAD
-- ---------------------------------------------------------------------
-- * IDEMPOTENT. The INSERT is guarded by the same NOT EXISTS it has
--   always had -- practice_instance_id + evidence_type_id +
--   source_obligation_id, Active. A second call inserts nothing.
-- * ADOPTED ONLY. @adopted reads practice_instance_obligation where
--   status = Active. An obligation nobody has adopted gets no rows; the
--   workspace shows its published list read-only instead. Adopting is
--   still the decision, and this does not pre-empt it.
-- * REPOSITORY ONLY. obligation_id > 0 excludes organization-defined
--   obligations, whose evidence travels the 231/232 link column instead.
-- * NO RESULT SET. It ends with the INSERT, so a plain EXEC returns
--   nothing to the caller. No INSERT ... EXEC, nothing to drain, and
--   nothing that could block a ROLLBACK.
-- * NOTHING IS DELETED OR OVERWRITTEN. It only creates what is missing
--   and claims rows that belong to no obligation. Removing evidence when
--   an obligation is un-adopted stays in sp_resolve_obligation_adopt,
--   where the decision to un-adopt is made.
--
-- SAFE TO RE-RUN. Requires 244.
-- NO SCHEMA CHANGE.
-- Rollback: 304_evidence_reconcile_on_load_rollback.sql
-- ASCII-only.
-- =====================================================================
SET NOCOUNT ON;
SET NOEXEC OFF;
...
BEGIN
    PRINT 'ABORT (304): sp_resolve_obligation_adopt missing -- run 244 first.';
    SET NOEXEC ON;
END
GO
...
BEGIN
    PRINT 'ABORT (304): vw_pm_obligation_evidence missing -- run 144 first.';
    SET NOEXEC ON;
END
GO

-- =====================================================================
-- 1. sp_resolve_evidence_reconcile_for_instance
--
--    The master lookups below are lifted from sp_resolve_obligation_adopt
--    unchanged, including their THROW numbers, so a database missing that
--    seed data fails the same way through either entry point.
-- =====================================================================
...
-- =====================================================================
-- 2. sp_resolve_obligation_adopt -- 244's body, with the evidence block
--    replaced by an EXEC of the procedure above. Nothing else changed.
-- =====================================================================
...
-- Sanity report
-- =====================================================================
SELECT 'sp_resolve_evidence_reconcile_for_instance present' AS Check_,
       CASE WHEN OBJECT_ID('grac_practice.sp_resolve_evidence_reconcile_for_instance','P') IS NOT NULL
            THEN 'PASS' ELSE 'FAIL' END AS Result
UNION ALL
SELECT 'adopt no longer holds its own evidence INSERT',
       CASE WHEN EXISTS (SELECT 1 FROM sys.sql_modules
                         WHERE object_id = OBJECT_ID('grac_practice.sp_resolve_obligation_adopt')
                           AND definition LIKE '%sp_resolve_evidence_reconcile_for_instance%')
            THEN 'PASS' ELSE 'FAIL' END;
GO

PRINT '304 evidence reconcile installed.';
GO
