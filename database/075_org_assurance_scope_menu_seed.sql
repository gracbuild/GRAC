-- =====================================================================
-- 075 Organization Assurance Scope Builder -- menu + permissions
--
-- Adds the second Phase 2 screen (BRD Part 2 Sec 2) under the existing
-- nav-assurance parent (migration 063), using the mandatory
-- parent_menu_id relationship established by migration 052.
--
-- Child menus added:
--   org-assurance-scope-builder   Assurance Scope Builder
--
-- Idempotent (MERGE on menu_key + feature_code).
-- Rollback: database/075_org_assurance_scope_menu_seed_rollback.sql.
-- =====================================================================
SET NOCOUNT ON;
SET NOEXEC OFF;
GO
...
BEGIN PRINT 'ABORT (075): schema grac_practice missing.'; SET @prereqs_ok = 0; END

IF OBJECT_ID('grac_practice.menu_master','U') IS NULL
BEGIN PRINT 'ABORT (075): menu_master missing.'; SET @prereqs_ok = 0; END

IF OBJECT_ID('grac_practice.feature_flag_master','U') IS NULL
   OR OBJECT_ID('grac_practice.feature_flag','U') IS NULL
BEGIN PRINT 'ABORT (075): feature_flag* missing. Run 041.'; SET @prereqs_ok = 0; END

IF NOT EXISTS (SELECT 1 FROM grac_practice.menu_master WHERE menu_key = N'nav-assurance')
BEGIN
    PRINT 'ABORT (075): parent menu nav-assurance not present. Run 063.';
    SET @prereqs_ok = 0;
END

IF @prereqs_ok = 0
BEGIN
    RAISERROR('075_org_assurance_scope_menu_seed: prerequisites missing.', 16, 1);
    SET NOEXEC ON;
END
GO
...
USING (VALUES
    (N'screen.org-assurance-scope-builder',
     N'Assurance Scope Builder',
     N'Phase 2 Assurance Management -- Scope Builder (BRD Part 2 Sec 2).')
...
VALUES
    (src.feature_code, src.feature_name, src.description, N'Screen', 0, 'seed-075');
GO
...
USING (VALUES
    (N'org-assurance-scope-builder',
     N'Scope Builder',
     N'Practice/org-assurance-scope-builder',
     461, N'diagram-project', N'Assurance')
...
VALUES
    (source.menu_key, source.menu_name, source.menu_url, source.display_order,
     source.icon_class, source.module_type, N'Active', 'seed-075');
GO

-- parent_menu_id wiring -- mandatory grouping mechanism.
DECLARE @nav_id BIGINT = (
    SELECT menu_id FROM grac_practice.menu_master WHERE menu_key = N'nav-assurance');

IF @nav_id IS NULL
    THROW 75001, 'nav-assurance parent menu missing. Run migration 063 first.', 1;

UPDATE grac_practice.menu_master
   SET parent_menu_id = @nav_id,
       updated_by     = 'seed-075',
       updated_dt     = SYSUTCDATETIME()
 WHERE menu_key IN (N'org-assurance-scope-builder');
GO
...
USING (
    ...
    WHERE r.role_name = N'Admin'
      ...
      AND m.menu_key IN (N'org-assurance-scope-builder')
) AS source
...
VALUES
    (source.role_id, source.menu_id, source.can_view, source.can_add,
     ...
     N'Active', source.record_status_id, 'seed-075');
GO

-- Per-org feature flag enable.
...
USING (
    ...
    WHERE o.status = N'Active'
      AND fm.feature_code IN (N'screen.org-assurance-scope-builder')
) AS source
ON target.organization_id = source.organization_id
   AND target.feature_flag_id = source.feature_flag_id
WHEN MATCHED THEN UPDATE SET
    is_enabled = 1,
    notes      = COALESCE(target.notes, N'Enabled by seed-075'),
    updated_by = 'seed-075',
    updated_dt = SYSUTCDATETIME()
WHEN NOT MATCHED THEN INSERT
...
VALUES
    (source.organization_id, source.feature_flag_id, 1,
     N'Enabled by seed-075', 'seed-075');
GO

COMMIT TRAN;
GO

SELECT 'org-assurance-scope-builder has parent_menu_id = nav-assurance' AS Check_,
       CASE WHEN EXISTS (
           SELECT 1 FROM grac_practice.menu_master c
           JOIN grac_practice.menu_master p ON p.menu_id = c.parent_menu_id
           WHERE c.menu_key = N'org-assurance-scope-builder'
             AND p.menu_key = N'nav-assurance')
            THEN 'PASS' ELSE 'FAIL' END AS Result;

PRINT '075 org-assurance-scope-builder menu seed complete.';
GO
