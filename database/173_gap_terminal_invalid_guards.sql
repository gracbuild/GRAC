-- =====================================================================
-- 173 Guard sp_custom_gap_analysis_save + sp_custom_gap_downstream_link_add
--
-- WHY
-- ---
-- Sir's rule: a gap marked Invalid or Duplicate during validation does
-- NOT need analysis, and no downstream artefact (task/exception/risk)
-- should ever be materialised for it. The existing UI let analysts open
-- Analysis on any state -- easy foot-gun. Adding the guard at the proc
-- layer as well so a rogue caller cannot bypass the UI.
--
-- "Terminal invalid" is defined as any gap_lifecycle_state_master row
-- with is_terminal=1 AND is_valid_terminal=0. Today that means
-- 'Invalid' and 'Duplicate'; tomorrow if operators add more such
-- states (e.g., 'Merged', 'Withdrawn'), the guard automatically covers
-- them without a code change.
--
-- BEHAVIOUR:
--   sp_custom_gap_analysis_save        -> THROW 55142
--   sp_custom_gap_downstream_link_add  -> THROW 55134
--
-- Rollback: 173_gap_terminal_invalid_guards_rollback.sql (restores 172
--           analysis proc and 156-era downstream_link_add proc without
--           the terminal-invalid check).
-- =====================================================================
SET NOCOUNT ON;
GO

-- ---------------------------------------------------------------------
-- 1. Guarded sp_custom_gap_analysis_save (extends 172 with a prelude
--    check for terminal-invalid state).
-- ---------------------------------------------------------------------
...
BEGIN
    ...
    -- Guard: refuse when the gap sits in a terminal-invalid state.
    -- Analyst has already decided the gap is not real (Invalid) or is
    -- already tracked elsewhere (Duplicate); analysis makes no sense
    -- and would fire misleading downstream auto-triggers.
    IF EXISTS (
        SELECT 1
...
-- ---------------------------------------------------------------------
-- 2. Guarded sp_custom_gap_downstream_link_add. Extend 157's body with
--    the same terminal-invalid check.
-- ---------------------------------------------------------------------
...
BEGIN
    ...
    IF EXISTS (
        SELECT 1
         ...
         WHERE g.custom_gap_id = @custom_gap_id
           AND s.is_terminal = 1 AND s.is_valid_terminal = 0)
        THROW 55134, 'sp_custom_gap_downstream_link_add: gap is in a terminal-invalid state (Invalid/Duplicate); downstream links are not applicable.', 1;
...
-- ---------------------------------------------------------------------
-- 3. Extend sp_custom_gap_header to return the two lifecycle-flag
--    columns so the UI can apply the same "terminal-invalid" rule as
--    the server without hard-coding state names client-side.
-- ---------------------------------------------------------------------
...
PRINT '173 gap terminal-invalid guards ready.';
GO
