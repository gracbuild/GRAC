-- =====================================================================
-- 208 Default password provisioning -- ROLLBACK
--
-- Restores sp_org_user_save and sp_org_user_list to their 133
-- definitions, byte-for-byte, and drops sp_org_user_set_password.
--
-- >>> REVERT THE APPLICATION FIRST <<<
-- Roll PracticeManagement.Web back to the build that still renders the
-- Password box on the Users form BEFORE running this. After this script
-- the create path once again requires $.passwordHash, so a Web build that
-- no longer collects a password will fail every user create with SQL
-- error 51152 -- the exact defect 208 was written to close.
--
-- WHY THE DATA IS LEFT ALONE
-- --------------------------
-- force_password_change belongs to migration 032, not to 208, and the
-- org-admin provisioning in 034 writes it independently. Blanking it here
-- would silently clear pending forced changes for admins provisioned by
-- 034 as well, so the column and its values stay exactly as they are.
--
-- WHAT YOU LOSE: users created under 208 keep force_password_change = 1
-- in the table, but nothing reads it once the Web tier is reverted --
-- those accounts will sign in on the shared default password with no
-- change prompt. Run the report at the foot of this script and reset
-- those passwords by hand.
-- =====================================================================
SET NOCOUNT ON;
SET NOEXEC OFF;
GO
...
BEGIN
    PRINT '208-rollback: sp_org_user_save missing -- 133 was never applied. Nothing to do.';
    SET NOEXEC ON;
END
GO

IF OBJECT_ID('grac_practice.sp_org_user_set_password','P') IS NOT NULL
    DROP PROCEDURE grac_practice.sp_org_user_set_password;
GO


-- =====================================================================
-- Restored from 133_org_party_and_team_type.sql
-- =====================================================================
...
PRINT '--- Accounts left holding a default password with no enforcement ---';
...
PRINT '208 rollback complete. sp_org_user_save / sp_org_user_list restored to their 133 definitions.';
GO
