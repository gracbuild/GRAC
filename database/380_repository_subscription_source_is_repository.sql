-- =====================================================================
-- 380 Release Owner -> Source reads Repository, not Manual
--
-- SIR'S INSTRUCTION
-- ------------------
--   The Source field under Release Owner was displaying "Manual" for
--   every record; it should display "Repository". Trace the actual
--   source of the value through database -> API -> frontend, confirm
--   whether "Manual" is hard-coded, and fix it at the correct layer --
--   not with a frontend-only relabel.
--
-- ROOT CAUSE, TRACED BEFORE WRITING ANYTHING
-- --------------------------------------------
--   The "Update Release Owner" dialog's Source field
--   (wwwroot/js/practice.js, openReleaseOwnerForm) reads:
--     valueOf(release, "SubscriptionType") || (releaseIdRaw < 0 ? "Custom" : "Central")
--   SubscriptionType comes straight from repository_subscription.subscription_type
--   (PracticeRepositoryService.cs's QuerySubscribedFrameworksAsync:
--   COALESCE(sub.subscription_type, N'Central') SubscriptionType -- the
--   'Central' fallback only fires on NULL, and the column is NOT NULL,
--   so it never actually fires). No C# controller or model file
--   contains any "SubscriptionType"/"Manual" default -- confirmed by
--   grep across Controllers/ and Models/ -- so the value is not
--   hard-coded in the API layer either.
--
--   It IS hard-coded, one layer further down: in
--   dbo.pm_manage_practice_repository's 'organization-setup' branch,
--   the INSERT that subscribes an organization to the Repository
--   releases selected during Organization Setup
--   (grac_new.release/grac_new.artifact -- the central Repository
--   catalog) writes the literal 'Manual' into subscription_type for
...
--            'Manual', 'Active', ...
--   This is the ONLY INSERT into repository_subscription that supplies
--   subscription_type for a real (non-Custom) Repository release --
--   confirmed by grepping every INSERT into repository_subscription
--   across database/*.sql. Every subscription an organization holds to
--   an actual Repository release was created here, so the reported
--   symptom ("Manual" for all records) is exactly what this line
--   produces on every run.
--
--   The one other INSERT (the generic, technical 'repository-subscriptions'
--   admin entity form -- wwwroot/js/practice.js, its own Subscription
--   Type dropdown offering Automatic/Manual as a real, user-picked
--   distinction) is NOT this bug: its menu row ('Repository
--   Subscriptions (Admin)', 274_menu_master_seed.sql) is seeded
--   Inactive and unreachable from the live navigation, so none of
--   sir's "for all records" rows came from it. Left untouched -- see
--   "What deliberately did not change" below.
--
--   The column's own NOT NULL DEFAULT 'Manual'
--   (001_practice_management_schema.sql) is never actually the source
--   either -- both known INSERTs supply an explicit value, so the
--   column default is dead in practice. Left as-is; it is not the
--   cause, and changing a column default would not affect either
--   INSERT.
--
-- WHAT THIS DOES
-- --------------
--   1. Re-issues dbo.pm_manage_practice_repository (CREATE OR ALTER
--      requires the whole body) with exactly one literal changed: the
--      organization-setup release-subscription INSERT now writes
--      'Repository' instead of 'Manual'. Every other branch of this
--      proc -- practices, dependencies, evidence, the generic
--      repository-subscriptions entity, everything else -- is
--      reproduced byte-for-byte from the current live definition (one
--      line differs; verified programmatically, not retyped by hand).
--   2. Backfills every EXISTING affected row so already-onboarded
--      organizations show the fix immediately, not only on their next
--      Organization Setup save:
--        UPDATE repository_subscription SET subscription_type='Repository'
--        WHERE release_id IS NOT NULL AND subscription_type='Manual';
--      release_id IS NOT NULL is exactly how the app already
--      distinguishes a real Repository-linked subscription from a
--      Custom one (Custom releases always have release_id NULL --
--      see PracticeRepositoryService.cs's two queries), so this cannot
--      touch a Custom row. It only touches rows that are currently
--      exactly 'Manual'; a row some admin genuinely set to a different
--      value via the (inactive, unreachable) generic form is left at
--      that value, unchanged.
--
-- WHAT THIS DELIBERATELY DOES NOT CHANGE
-- -----------------------------------------
--   * No frontend file. practice.js's Source field already displays
--     whatever SubscriptionType the API returns; once the database
--     value is correct, the field is correct with no UI edit -- exactly
--     what sir asked for ("update the appropriate source/mapping
--     instead of hard-coding Repository in the UI").
--   * No C#/API file. PracticeRepositoryService.cs's SELECT already
--     passes subscription_type straight through (only defaulting a
--     genuinely NULL value, which does not occur here); no model or
--     controller file held a competing default -- confirmed by grep,
--     no edit needed.
--   * The generic 'repository-subscriptions' admin entity screen
--     (practice.js, its Subscription Type Automatic/Manual dropdown) --
--     inactive menu row, unreachable, not this bug's source. Left
--     exactly as-is.
--   * subscription_type's NOT NULL DEFAULT 'Manual'
--     (001_practice_management_schema.sql) -- dead in practice, not
--     touched.
--   * Custom releases (subscription_type='Custom', release_id NULL) --
--     structurally excluded from the backfill by release_id IS NOT NULL,
--     and no line in the reissued proc's Custom-release handling
--     changed.
--
-- DEPENDS ON: 001 (repository_subscription), 002 (this proc, base
-- definition being re-issued).
-- Rollback: 380_repository_subscription_source_is_repository_rollback.sql
-- Re-runnable: yes -- CREATE OR ALTER; the backfill UPDATE is
-- naturally idempotent (WHERE subscription_type='Manual' matches
-- nothing once already applied).
-- =====================================================================
SET NOCOUNT ON;
SET XACT_ABORT ON;
SET NOEXEC OFF;
...
BEGIN
    RAISERROR('ABORT (380): grac_practice.repository_subscription missing. Run 001 first.', 16, 1);
    SET NOEXEC ON;
END
GO
...
BEGIN
 ...
 BEGIN
   ...
   SELECT @new_id,a.authority_id,a.artifact_id,r.release_id,'Repository','Active',@active_subscription_status_id,CONVERT(DATE,SYSUTCDATETIME()),'Active',@active_record_status_id,@p_usr_id
   FROM @selected_releases x
...
PRINT '380: pm_manage_practice_repository re-issued -- Repository-sourced subscriptions now record subscription_type = Repository.';
GO
...
UPDATE grac_practice.repository_subscription
   SET subscription_type = N'Repository',
       updated_by        = N'seed-380',
       updated_dt         = SYSUTCDATETIME()
 WHERE release_id IS NOT NULL
   AND subscription_type = N'Manual';

PRINT '380: backfilled ' + CAST(@@ROWCOUNT AS NVARCHAR(20)) + ' existing repository_subscription row(s) from Manual to Repository.';
GO

-- =====================================================================
-- Verification
-- =====================================================================
SELECT '380-a proc compiled' AS Check_,
       CASE WHEN OBJECT_DEFINITION(OBJECT_ID('dbo.pm_manage_practice_repository','P')) IS NOT NULL
            THEN 'PASS' ELSE 'FAIL' END AS Result
UNION ALL
SELECT '380-b org-setup subscription INSERT writes Repository, not Manual',
       CASE WHEN OBJECT_DEFINITION(OBJECT_ID('dbo.pm_manage_practice_repository','P')) LIKE '%r.release_id,''Repository''%'
             AND OBJECT_DEFINITION(OBJECT_ID('dbo.pm_manage_practice_repository','P')) NOT LIKE '%r.release_id,''Manual''%'
            THEN 'PASS' ELSE 'FAIL' END
UNION ALL
SELECT '380-c no Repository-linked subscription still reads Manual',
       CASE WHEN NOT EXISTS (
                SELECT 1 FROM grac_practice.repository_subscription
                 WHERE release_id IS NOT NULL AND subscription_type = N'Manual')
            THEN 'PASS' ELSE 'FAIL' END
UNION ALL
SELECT '380-d Custom releases untouched (Custom rows still have release_id NULL)',
       CASE WHEN NOT EXISTS (
                SELECT 1 FROM grac_practice.repository_subscription
                 WHERE subscription_type = N'Custom' AND release_id IS NOT NULL)
            THEN 'PASS' ELSE 'FAIL' END;

PRINT '';
PRINT '--- 380 diagnostic: current subscription_type distribution ---';
SELECT subscription_type AS SubscriptionType,
       CASE WHEN release_id IS NULL THEN N'Custom release' ELSE N'Repository release' END AS Kind,
       COUNT(*) AS RecordCount
  FROM grac_practice.repository_subscription
 GROUP BY subscription_type, CASE WHEN release_id IS NULL THEN N'Custom release' ELSE N'Repository release' END
 ORDER BY SubscriptionType;

PRINT '';
PRINT '380 complete. Release Owner -> Source now reads Repository for every';
PRINT '    subscription linked to an actual Repository release; Custom';
PRINT '    releases are unaffected. See';
PRINT '    docs/repository-subscriptions-grid.md (Update Release Owner section).';
GO
SET NOEXEC OFF;
GO
