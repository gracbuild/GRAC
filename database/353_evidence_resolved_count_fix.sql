-- =====================================================================
-- 353_evidence_resolved_count_fix.sql
--
-- SYMPTOM
--   In Operationalize, obligation + evidence now save together. Saving an
--   obligation with NO evidence details still shows the evidence as
--   "resolved" (green) -- e.g. a Not Implemented obligation with blank
--   evidence reads as resolved.
--
-- CAUSE
--   sp_resolve_obligation_list projects ResolvedEvidenceCount as
--   COUNT(*) of ACTIVE practice_instance_evidence rows -- it counted an
--   evidence row that merely EXISTS. The single obligation+evidence save
--   creates the evidence row up front (blank location/locator), so the
--   count jumped and the UI turned the obligation green. Everywhere else
--   (sp_resolve_evidence_list.IsResolved, the 232 verification) "resolved"
--   means location AND locator are BOTH filled.
--
-- FIX
--   Re-issues sp_resolve_obligation_list verbatim from 244 with both
--   ResolvedEvidenceCount subqueries (published + organisation-defined)
--   counting only rows where evidence_location AND evidence_locator are
--   both non-empty. Green now appears only once the evidence is actually
--   filled in.
--
-- SAFE TO RE-RUN (CREATE OR ALTER). ASCII-only.
-- Rollback: 353_evidence_resolved_count_fix_rollback.sql (restores 244's proc)
-- =====================================================================
SET NOCOUNT ON;
GO
...
PRINT '353: sp_resolve_obligation_list ResolvedEvidenceCount now requires location + locator.';
GO
