-- =====================================================================
-- 371 rollback -- restores sp_gap_centre_list to 357's exact body
-- (Practice/Task/Risk/Exception status columns removed).
...
BEGIN PRINT 'ABORT (371 rollback): sp_gap_centre_list missing.'; SET NOEXEC ON; END
GO
...
DECLARE @defrb NVARCHAR(MAX) = OBJECT_DEFINITION(OBJECT_ID('grac_practice.sp_gap_centre_list','P'));
SELECT '371-rollback sp_gap_centre_list compiled' AS Check_,
       CASE WHEN @defrb IS NOT NULL THEN 'PASS' ELSE 'FAIL' END AS Result
UNION ALL
SELECT '371-rollback new status columns removed',
       CASE WHEN @defrb NOT LIKE '%PracticeInstanceStatusText%' THEN 'PASS' ELSE 'FAIL' END;

PRINT '371 rollback complete. sp_gap_centre_list restored to 357''s body.';
GO
