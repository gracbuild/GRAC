...
(() => {
  ...
  function bindEvents() {
    ...
    // Migration 328 -- "Add to list" for Related Control / Practice(s).
    const addCustomPracticeBtn = document.getElementById("newExcPracticeAddBtn");
    if (addCustomPracticeBtn) addCustomPracticeBtn.addEventListener("click", onAddCustomPracticeClick);
  ...
  async function openAddCustomDialog() {
    ...
    // Migration 328 -- Related Control / Practice(s) is now the reusable
    // cascading Practice Picker plus an "Add to list" panel, not the flat
    // single-select loadPractices()/renderPracticeOptions() fed. Reset
    // the running list every time the dialog opens.
    addCustomPractices = [];
    renderAddCustomPracticeList();
    const addHint = document.getElementById("newExcPracticeAddHint");
    if (addHint) addHint.textContent = "";
    if (window.__practicePicker && document.getElementById("newExcPracticePickerHost")) {
      if (addCustomPicker) {
        // Same reuse-across-opens sequence Risk Centre's own mapPicker
        // singleton uses: retarget the organisation, drop any exclusion
        // left over from the last time this dialog was open (the list
        // was just reset to empty above), then reload the cascade from
        // the framework level.
        addCustomPicker.setOrganizationId(state.organizationId);
        addCustomPicker.setExcluded([]);
  ...
  // 328: one practice at a time from the picker, "Add to list" moves it
  // into addCustomPractices and resets the picker for the next pick --
  // the same shape as Risk Centre's own Map Practice "Add" step.
  function onAddCustomPracticeClick() {
    ...
    renderAddCustomPracticeList();
    // Keep already-added practices out of the next pick, and clear the
    // cascade so the analyst is not looking at what they just added.
    if (addCustomPicker.setExcluded) addCustomPicker.setExcluded(addCustomPractices.map(p => p.practiceId));
    if (addCustomPicker.reset) addCustomPicker.reset();
  }
